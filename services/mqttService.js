...
// PostGIS edition — replaces Haversine formula with ST_Within.
...
function notify() {
  if (!_notify) _notify = require('./notifyService');
  return _notify;
}
function log() {
...
// Returns true (inside), false (outside), or null (no geofence)
// ST_Within checks if the device point is inside the stored
// geofence geometry using the spatial index — no JavaScript math.
// =============================================================
...
// Find other active incidents within radiusMetres of this location
...
// =============================================================
// Read thresholds from system_config table
// =============================================================
...
// Topic: sfdaass/sensors/{deviceCode}
// =============================================================
async function handleSensorData(deviceCode, payload, io) {
  ...
  // 3. Push live update to dashboard
  if (io) {
  ...
  // 5. PostGIS geofence check — replaces Haversine
  ...
  // 7. Auto-sprinkler for critical events
  ...
  notify().sendSMS(msg).then(function(ok) {
...
// Topic: sfdaass/status/{deviceCode}
// =============================================================
...
// =============================================================
// Handle GPS update
// Topic: sfdaass/gps/{deviceCode}
// Updates both plain lat/lng AND the PostGIS location point
// =============================================================
