...
module.exports = {
  ...
  getClient: getClient,
  pushConfigToDevice: pushConfigToDevice,  // FIX: newly exported for device config push
  pushConfigToAll: pushConfigToAll,         // FIX: newly exported
...
// =============================================================
// FIX: Push configuration to a specific device (or all devices)
// Called by routes/devices.js → POST /api/devices/:code/config
// Previously missing — the dashboard had no way to push to the
// MQTT broker because no publish function was exported.
// =============================================================
function pushConfigToDevice(deviceCode, config) {
  if (!client) {
    log().error('[MQTT] Cannot push config — MQTT client not initialised');
    return false;
  }
  if (!client.connected) {
    log().error('[MQTT] Cannot push config to ' + deviceCode + ' — broker not connected');
    return false;
  }
  // retain=true so a device that reconnects still receives its latest config
  var topic   = 'sfdaass/config/' + deviceCode;
  var payload = JSON.stringify(Object.assign({ ts: new Date().toISOString() }, config));
  client.publish(topic, payload, { qos: 1, retain: true }, function(err) {
    if (err) log().error('[MQTT] Failed to push config to ' + deviceCode + ': ' + err.message);
    else     log().info('[MQTT] Config pushed → ' + topic);
  });
  return true;
}
