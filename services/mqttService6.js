...
// Handle plain-text alert messages from device (sfdaass/alert/<deviceCode>)
// Payload is plain text: "FIRE DETECTED" — NOT JSON
// Server-side fire detection already runs inside handleSensorData via sensor readings;
// this handler just logs the device-side alert and updates device last_seen.
...
/**
 * Publish threshold config to a physical device over MQTT.
 * Topic:   sfdaass/config/<deviceCode>   (retained, QoS 1)
 * Payload: JSON object with threshold keys, e.g.:
 *   { smoke_warning: 120, temp_warning: 45, gas_warning: 200 }
 *
 * The firmware must subscribe to this topic and apply the values.
 *
 * @param {string} deviceCode  e.g. "ESP8266-001"
 * @param {object} config      threshold key/value pairs
 * @returns {boolean}          true if MQTT accepted the publish
 */
function pushConfigToDevice(deviceCode, config) {
  if (!client || !client.connected) {
    log().warn('[pushConfig] MQTT client not available — config not sent');
    return false;
  }
  if (!client.connected()) {
    log().warn('[pushConfig] MQTT not connected — config not sent to ' + deviceCode);
    return false;
  }
  ...
  // QoS 1 (acknowledged) + retain so the device gets it even after a reconnect
  client.publish(topic, payload, { qos: 1, retain: true });
  log().info('[pushConfig] Config pushed to ' + deviceCode + ': ' + payload);
  return true;
}
...
module.exports = {
  ...
  getClient:          getClient,
  pushConfigToDevice: pushConfigToDevice,
